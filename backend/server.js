...
// Нативная обёртка (Capacitor, Android/iOS APK) шлёт запросы с этих локальных
// origin — всегда разрешаем их, иначе установленное приложение получает «Failed to fetch».
const APP_ORIGINS = ['https://localhost', 'http://localhost', 'capacitor://localhost', 'ionic://localhost'];
app.use(cors(process.env.NODE_ENV === 'production'
  ? { origin: [...allowed, ...APP_ORIGINS] }
  : {}));
