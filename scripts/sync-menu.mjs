...
//
// ⚠️ УСТАРЕЛ. Меню ведётся в backend/db.js и едет деплоем (досев, снятие, цены,
// переименования, поля). Прайс в этом файле отстал от печатной карты бара, а
// названия — от MENU_RENAME_TO: запуск создаст дубли уже существующих позиций.
// Прежде чем пользоваться — сверить список ниже с сидом в db.js.
//
// Запуск:
